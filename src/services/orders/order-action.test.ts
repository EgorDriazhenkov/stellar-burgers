...
describe('orders-actions', () => {
  ...
  describe('getFeedsThunk', () => {
    ...
    test('Запрос с ошибкой', async () => {
      jest
        ...
        .mockRejectedValue(new Error('Ошибка при получении ингредиентов'));
  ...
  describe('postUserBurgerThunk', () => {
    ...
    it('Ошибка в передаче заказа', async () => {
      jest
        .spyOn(api, 'orderBurgerApi')
        .mockRejectedValue(new Error('Ошибка в передаче заказа'));
  ...
  describe('getUserOrdersThunk', () => {
    ...
    it('Запрос с ошибкой', async () => {
      jest
        ...
        .mockRejectedValue(
          new Error('Ошибка в получении заказов пользователя')
        );
      const dispatch = await runThunk(getUserOrdersThunk());
      expect(dispatch).toHaveBeenCalledWith(
        expect.objectContaining({
